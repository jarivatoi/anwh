...
export const ShiftModal: React.FC<ShiftModalProps> = ({
  ...
  useEffect(() => {
    if (selectedDate) {
      ...
      console.log('🔵 [SHIFTMODAL INIT] Date:', selectedDate, 'isSpecial:', isSpecial, 'specialDateText:', specialDateTexts[selectedDate]);
      
      // Only update if different from current state (prevents resetting user's toggle)
      setIsSpecialDate(prev => {
        if (prev !== isSpecial) {
          console.log('🔄 [SHIFTMODAL] Updating isSpecialDate from', prev, 'to', isSpecial);
          return isSpecial;
        }
        console.log('⏭️ [SHIFTMODAL] Keeping current isSpecialDate:', prev);
        return prev;
      });
