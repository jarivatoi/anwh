...
export const StaffManagementModal = ({
  ...
  // Validate admin privilege - max 2 admins per institution (for admin 5274's posting)
  const validateAdminPrivilege = async (institutionCode: string | null, targetUserId?: string): Promise<boolean> => {
    ...
    // Count existing admins for this institution
    const existingAdmins = staffMembers.filter(
      staff => staff.institution_code === institutionCode && staff.is_admin && staff.id !== targetUserId
    );

    // Allow up to 2 admins per institution
    if (existingAdmins.length >= 2) {
      const adminNames = existingAdmins.map(a => `${a.name} (${a.id_number})`).join(', ');
      setAdminPrivilegeError(`Cannot grant admin privileges: Maximum 2 admins already assigned to ${institutionCode}: ${adminNames}`);
      return false;
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleFormChange('id_number', e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 14))}
                ...
                {!editingStaff && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setConfirmIdNumber(e.target.value.toUpperCase())}
                      disabled={!isAdminAuthenticated}
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono ${
                        formErrors.confirmIdNumber ? 'border-red-500' : 'border-gray-300'
                      }`}
                      placeholder="Re-enter staff ID"
                    />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleFormChange('surname', e.target.value.toUpperCase())}
                    disabled={!isAdminAuthenticated}
                  ...
                  <div>
                    <label className={`flex items-center space-x-2 text-sm font-medium ${
                      formData.institution_code && (() => {
                        // Count existing admins in selected institution (excluding Admin 5274)
                        ...
                        const maxAllowed = isPostedHere ? 2 : 1;
                        
                        // For display, include Admin 5274 in the count if posted here
                        const totalCount = adminCount + (isPostedHere ? 1 : 0);
                        
                        if (totalCount >= maxAllowed) {
                          return <span className="text-xs text-red-600 ml-2">(Max {maxAllowed} admin{maxAllowed > 1 ? 's' : ''} reached for {formData.institution_code})</span>;
                        } else if (totalCount > 0) {
                          return <span className="text-xs text-orange-600 ml-2">(Max {totalCount}/{maxAllowed} admin{maxAllowed > 1 ? 's' : ''})</span>;
                        }
                        return null;
                      })()
                      ...
                      <input
                        ...
                        disabled={!isAdminAuthenticated || !!(formData.institution_code && (() => {
                          ...
                          // Max allowed admins per institution is ALWAYS 1 (excluding Admin 5274)
                          // Admin 5274 doesn't count towards the limit - they're super admin
                          const maxAllowed = 1;
                      ...
                      {formData.institution_code && (() => {
                        ...
                        const isPostedHere = currentUser?.posting_institution === formData.institution_code;
                        const displayMaxAllowed = isPostedHere ? 2 : 1;
                        
                        // Calculate total for display: other admins + Admin 5274 (if posted here) + current edit if keeping as admin
                        let totalCount = adminCount;
                        if (isPostedHere) {
                          totalCount += 1; // Count Admin 5274
                        }
                        if (editingStaff?.is_admin === true && formData.is_admin === true) {
                          // Editing an existing admin and keeping them as admin - count them
                          totalCount += 1;
                        }
