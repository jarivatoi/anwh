...
export const PDFImportModal: React.FC<PDFImportModalProps> = ({
  ...
  const handleParsePDF = async () => {
    ...
    setParsing(true);
    try {
      console.log('🔍 Starting PDF parse with institution filtering...');
      ...
      if (userSession?.userId) {
        ...
        if (userData) {
          ...
          console.log('🔍 PDF Parse - User Institution:', userInstitution, 'Is Admin 5274:', isAdmin5274);
      ...
      if (institutionStaffNames.length > 0) {
        console.log('🎯 Setting filtered staff list on pdfRosterParser.boxParser...');
        pdfRosterParser.boxParser.setStaffNames(institutionStaffNames);
        console.log('✅ Staff list set! Parser will use these', institutionStaffNames.length, 'names only');
      } else {
        console.warn('⚠️ No institution staff list set - parser will use default availableNames');
      }
  ...
  const handleImport = async () => {
    ...
    console.log('📥 PDF Import starting...', logEntry);
    ...
    try {
      ...
      console.log('⏰ Scheduling modal close in 500ms...');
