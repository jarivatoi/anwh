...
export const RosterTableView: React.FC<RosterTableViewProps> = ({
  ...
  useEffect(() => {
    const fetchStaffData = async () => {
      try {
        console.log('👥 RosterTableView: Fetching staff data...');
        ...
        console.log('🏢 Current user institution from session:', userInstitution);
        ...
        if (!userInstitution && userSession?.userId) {
          ...
          try {
            ...
            if (error) {
              ...
              console.log('🏢 Current user institution from Supabase:', userInstitution);
            }
          } catch (err) {
        ...
        if (userInstitution) {
          ...
          console.log('🔒 Filtering by institution:', userInstitution);
        } else {
          console.log('🌐 No institution filter - showing ALL staff');
        }
        ...
        if (data && data.length > 0) {
          console.log(`✅ RosterTableView: Raw staff count (${userInstitution || 'all institutions'}):`, data.length);
          console.log('📋 All staff from database:', data.map((s: any) => `${s.surname}, ${s.name} (${s.institution_code})`));
          ...
          console.log('✅ RosterTableView: Final staff count (with R variants):', sortedNames.length);
