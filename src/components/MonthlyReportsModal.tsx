...
export const MonthlyReportsModal: React.FC<MonthlyReportsModalProps> = ({
  ...
  const loadStaffForMonth = async () => {
    ...
    // Extract unique assigned names from roster entries (formatted for display)
    const staffSet = new Set<string>();
    monthEntries.forEach(entry => {
      // Use formatDisplayNameForUI to strip ID number first, then apply smart (R) stripping
    ...
    if (!userInstitution) {
      ...
      setAvailableStaff(staffArray.map(name => ({ displayName: name, surname: name })));
      return;
    }
