...
function refreshDerivedArrays(): void {
  ...
  const newAvailableNames = authCodes
    ...
    .sort((a, b) => {
      ...
      // Normal names first, (R) names after
...
export let availableNames = authCodes
  ...
  .sort((a, b) => {
    ...
    // Priority 3: Within same title and asterisk status, normal names first, then (R)
    if (titleA === titleB) {
      const aHasR = a.includes('(R)');
      const bHasR = b.includes('(R)');
      
      // Normal names come first, (R) names after
      if (aHasR && !bHasR) return 1;
      if (!aHasR && bHasR) return -1;
...
export const sortRosterEntriesByGroup = (entries: RosterEntry[]): RosterEntry[] => {
  return [...entries].sort((a, b) => {
    ...
    // Priority 3: Within same title and center status, normal names first, then (R)
    if (titleA === titleB) {
