  ...
  private findMatchingStaffName(text: string): string | null {
    ...
    for (const nameUpper of availableNames) {
      ...
      if (commaIndex > 0) {
        ...
        if (cleanText === surnameOnly && surnameOnly.length >= 3) {
          ...
          return nameUpper; // Return full name from database
        }
      }
    }
    ...
    for (const nameUpper of availableNames) {
      ...
      if (cleanTextBase === baseName && baseName.length >= 3) {
        console.log(`✅ BASE NAME MATCH: "${cleanText}" matches "${nameUpper}" (base: "${baseName}")`);
        return nameUpper; // Return the full ID-based format
      }
