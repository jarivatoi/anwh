...
export const syncRosterToCalendar = (
  ...
  // Check for conflicts - but be more lenient for import
  const hasConflict = checkShiftConflicts(date, shiftType, currentShifts);
  if (hasConflict) {
    console.log(`⚠️ ROSTER SYNC: Shift conflict detected for ${calendarShiftId} on ${date} with existing shifts:`, currentShifts, '- but proceeding anyway for import');
  }
  
  // Apply changes to calendar
  let calendarUpdated = false;
  
  // Mark as special date if needed
  if (needsSpecial && !currentIsSpecial) {
    console.log(`✅ ROSTER SYNC: Marking ${date} as special date`);
    setSpecialDates(prev => ({
      ...prev,
  ...
  if (calendarUpdated) {
    console.log(`✅ ROSTER SYNC: Calendar updated successfully for ${date} - added ${calendarShiftId}`);
  }
