  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        console.log('🔍 Found potential name change:', {
      ...
      if (
        ...
        console.log('🔄 Found potential swap/exchange:', entry);
        return {
          found: true,
          originalName: entry.assigned_name,
          changeDescription: entry.change_description
...
export const addRosterEntry = async (formData: RosterFormData, editorName: string): Promise<RosterEntry> => {
  ...
  try {
    ...
    // Check for duplicate entries (same date, shift, and staff member)
    ...
    if (existingEntries && existingEntries.length > 0) {
      console.log('⏭️ Skipping duplicate entry:', formData.assignedName, 'on', formData.date, formData.shiftType);
      // Return existing entry instead of throwing error - allows import to continue
      return { ...existingEntries[0], shiftType: existingEntries[0].shift_type, assignedName: existingEntries[0].assigned_name } as RosterEntry;
    }
    ...
    // Also dispatch rosterUpdated to trigger special date re-sync
    window.dispatchEvent(new CustomEvent('rosterUpdated', {
      detail: { type: 'addition', entry: data }
    }));
