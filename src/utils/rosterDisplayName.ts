 ...
 * Strips ID number but preserves (R) suffix and other annotations
 */
export function formatDisplayNameForUI(displayName: string): string {
  // Extract (R) suffix if present - keep it for display
  const hasR = displayName.includes('(R)');
  let cleanName = displayName.replace(/\(R\)/g, '').trim();
  
  const parts = cleanName.split('_');
  
  if (parts.length < 2) {
    return hasR ? `${cleanName}(R)` : cleanName;
  }
  ...
  if (hasMiddlePart) {
    ...
    return hasR ? `${result}(R)` : result;
  } else {
    const surname = parts.slice(0, parts.length - 1).join('_');
    return hasR ? `${surname}(R)` : surname;
  }
}
