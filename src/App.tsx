...
const MainApp: React.FC<{ user: UserSession | null; onLogout: () => void; onLoginSuccess: (sess: { userId: string; idNumber: string; isAdmin: boolean }) => void }> = ({ user, onLogout, onLoginSuccess }) => {
  ...
  const { schedule, specialDates, dateNotes, setSchedule, setSpecialDates, setDateNotes, error: dataError } = useScheduleData();
  ...
  // Sync roster special dates to calendar - ONLY SYNC IF NO EXISTING TEXT
  const syncRosterSpecialDatesToCalendar = useCallback(async () => {
    try {
      ...
      if (Object.keys(specialDateFlags).length > 0) {
        ...
        // MERGE with existing special dates - ONLY ADD IF NO EXISTING TEXT
        setSpecialDates(prev => {
          const merged = { ...prev };;
          Object.entries(specialDateFlags).forEach(([date, isSpecial]) => {
            // Only add if date doesn't exist OR if it exists but has no text yet (blank)
            if (!(date in merged)) {
        ...
        // Only update texts for dates that don't have text yet
        setSpecialDateTexts(prev => {
          const updated = { ...prev };
          Object.entries(specialDateTextMap).forEach(([date, text]) => {
            // Only add text if it doesn't exist or is blank/"SPECIAL"
            const existingText = prev[date];
            if (!existingText || existingText === 'SPECIAL') {
              updated[date] = text;
            } else {
            }
          });
  ...
  useEffect(() => {
    ...
    // CRITICAL: Skip sync if specialDates hasn't loaded yet (still empty object)
    // This prevents roster from overwriting manually added special dates
    ...
    const runSync = async () => {
      try {
        ...
        if (Object.keys(rosterSpecialDates).length > 0) {
          // MERGE strategy: Combine roster dates + manual SPECIAL dates
          setSpecialDates(prev => {
            const merged = { ...prev };
            let changed = false;
            ...
            Object.entries(rosterSpecialDates).forEach(([date, isSpecial]) => {
              if (!(date in merged)) {
                // Roster has it but we don't - add it
                merged[date] = isSpecial;
                changed = true;
              }
            });
          ...
          // For texts: ALWAYS use roster text for roster dates (overwrites SPECIAL placeholders)
          setSpecialDateTexts(prev => {
            const updated = { ...prev };
            let changed = false;
            
            Object.entries(rosterTextMap).forEach(([date, rosterText]) => {
              const currentText = prev[date];
              
              // Always use roster text for roster special dates
              if (currentText !== rosterText) {
                updated[date] = rosterText;
                changed = true;
              }
            });
  ...
  }, [specialDates]); // Re-run when specialDates loads from IndexedDB
