...
const MainApp: React.FC<{ user: UserSession | null; onLogout: () => void; onLoginSuccess: (sess: { userId: string; idNumber: string; isAdmin: boolean }) => void }> = ({ user, onLogout, onLoginSuccess }) => {
  ...
  const syncRosterSpecialDatesToCalendar = useCallback(async () => {
    try {
      ...
      // SYNC special dates - ADD new ones, REMOVE ones no longer in roster
      setSpecialDates(prev => {
        const synced = { ...prev };
        
        // Remove dates that are no longer in roster
        Object.keys(synced).forEach(date => {
          if (!(date in specialDateFlags)) {
            delete synced[date];
          }
        });
        
        // Add new dates from roster
        Object.entries(specialDateFlags).forEach(([date, isSpecial]) => {
          synced[date] = isSpecial;
        });
        
        return synced;
      });
      
      // SYNC special date texts - ADD new ones, REMOVE ones no longer in roster
      setSpecialDateTexts(prev => {
        const synced = { ...prev };
        
        // Remove texts for dates no longer in roster
        Object.keys(synced).forEach(date => {
          if (!(date in specialDateTextMap)) {
            delete synced[date];
          }
        });
        
        // Add/update texts from roster
